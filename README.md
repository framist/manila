...
- **单船到达概率**：利用动态规划表 \(\text{DP}[i][t]\) 计算平底船在剩余 \(t\) 次骰子时从位置 \(i\) 出发越过阈值的概率。其中
  \(\Pr(X_i > 12)\) 与 \(\Pr(X_i > 13)\) 分别对应“无海盗”与“海盗存在”时的到港条件。
- **显式海盗状态**：对每艘船 \(i\) 定义
  \[
  P_i^{(\text{pirate})} = \Pr(X_i > 13),\qquad
  P_i^{(\text{safe})} = \Pr(X_i > 12).
  \]
  若有玩家实际占据海盗位置，则全局海盗概率 \(p_{\text{pirate}} = 1\)；否则 \(p_{\text{pirate}}\) 取自滑杆设置。
- **联合分布**：使用动态规划函数 \(f_m(\mathbf{P})\) 计算给定概率向量 \(\mathbf{P}\) 时恰有 \(m\) 艘船到港的概率。最终结果按全局海盗概率加权：
  \[
